...
import { Gavel, Scale, Landmark, Home, ClipboardList, Loader2, AlertCircle } from "lucide-react";
...
const COURT_STATS_API_URL = "http://localhost:3001/api/v1/admin/courts/statistics";
...
const CourtStatistics = () => {
    ...
    const { data: stats, isLoading, error } = useQuery<Record<CourtType, CourtStats>>({
        ...
        queryFn: async () => {
            ...
            const validatedData: Record<CourtType, CourtStats> = {} as Record<CourtType, CourtStats>;
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 p-4">
            {(Object.entries(stats) as [CourtType, CourtStats][]).map(([courtType, courtStats]) => (
                <CourtCard
                    key={courtType}
                    courtType={courtType}
                    stats={courtStats}
...
function CourtCard({ courtType, stats }: { courtType: CourtType; stats: CourtStats }) {
