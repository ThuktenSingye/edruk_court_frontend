...
const COURT_TYPES_API_URL = "http://localhost:3001/api/v1/admin/courts/court_types";
...
const ParentCourtSelect = ({
    ...
    const handleChange = (id: string) => {
        const selectedCourt = filteredCourts.find((court) => court.id === parseInt(id));
        onChange(
            selectedCourt
                ? {
                    id: selectedCourt.id,
    ...
    if (!filteredCourts.length) {
        return <div className="text-sm text-gray-500">No parent court required</div>;
    }
...
const CourtTable = () => {
    ...
    const editCourtMutation = useMutation({
        ...
        onError: (error: Error) => {
            toast.error(error.message || "Failed to update court", { duration: 5000 });
        }
    });
    ...
    const deleteCourtMutation = useMutation({
        ...
        onError: (error: Error) => {
            toast.error(error.message || "Failed to delete court", { duration: 5000 });
        }
    });
        ...
        () => [
            ...
            { accessorKey: "name", header: "Court Name" },
            {
                ...
                cell: ({ row }) => {
                    ...
                    return (
                        <span
                            className={`px-2 py-1 rounded-full text-xs font-medium ${colorMap[type] || "bg-gray-100"
                                }`}
                        >
                        ...
                        <ParentCourtSelect
                            ...
                            onChange={(parentCourt) => {
                                if (editingCourt) {
                                    setEditingCourt({
                                        ...editingCourt,
                                        parent_court: parentCourt || { id: null, name: null, type: null },
                                    });
                                } else {
                                    setNewCourt({
                                        ...newCourt,
