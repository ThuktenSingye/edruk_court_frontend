...
export default function Bench() {
    ...
    useEffect(() => {
        async function fetchData() {
            const response = await fetch("http://localhost:3002/bench1");
            const stats: CaseStat[] = await response.json();
            setCaseStats(stats);

            const officialsResponse = await fetch("http://localhost:3002/bench1officials");
    ...
    return (
        <div className="p-6">
            <h2 className="text-xl font-bold mb-4">Bench I</h2>
