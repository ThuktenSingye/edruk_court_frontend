...
const CaseReportComponent = () => {
    ...
    const { data: caseStats, isLoading, error } = useQuery<ApiResponse['data']>({
        ...
        queryFn: async () => {
            ...
            const response = await fetch(`http://${host}:3001/api/v1/case_statistics?year=${selectedYear}`, {
                headers: {
            ...
            <div className="mb-4 flex gap-4">
                ...
                >
                    {availableBenches.map((bench) => (
                        <option key={bench} value={bench}>Bench {bench}</option>
                    ))}
                </select>
                <select
                    value={selectedMonth}
                                ...
                                Object.entries(caseStats.court_case_statistic[selectedBench])
                                    ...
                                    .map(([month, monthData]) => (
                                        <React.Fragment key={month}>
                                            {Object.entries(monthData).map(([benchClerk, stats], index) => (
                                                <TableRow key={`${month}-${benchClerk}`}>
                                                    {index === 0 ? (
                                                        <TableCell rowSpan={Object.keys(monthData).length} className="align-middle">
                                                            {month}
                                                        </TableCell>
                                                    ) : null}
                                            ...
                                            <TableRow className="bg-gray-50 font-semibold">
                                                ...
                                                <TableCell className="text-center">
                                                    {Object.values(monthData).reduce((sum, stats) => sum + stats.opening_balance, 0)}
                                                </TableCell>
                                                <TableCell className="text-center">
                                                    {Object.values(monthData).reduce((sum, stats) => sum + stats.registered, 0)}
                                                </TableCell>
                                                <TableCell className="text-center">
                                                    {Object.values(monthData).reduce((sum, stats) => sum + stats.decided, 0)}
                                                </TableCell>
                                                <TableCell className="text-center">
                                                    {Object.values(monthData).reduce((sum, stats) => sum + stats.pending, 0)}
                                                </TableCell>
                                                <TableCell className="text-center">
                                                    {Object.values(monthData).reduce((sum, stats) => sum + stats.appeal, 0)}
                                                </TableCell>
                                            </TableRow>
                                        </React.Fragment>
                                    ))
