...
const Documents: React.FC<DocumentsProps> = ({
    ...
    const handleSign = async (documentId: number) => {
        ...
        try {
            const endpoint = hearingType?.toLowerCase() === "miscellaneous"
            ...
            alert(err.message || "Something went wrong while signing the document. Please try again.");
        } finally {
            setSigningOne(null);
        }
    };
    ...
    const handleSignAll = async () => {
        ...
        try {
            ...
            if (responseData.status === "ok" && responseData.data) {
                setLocalDocs((prevDocs) =>
                    prevDocs.map((doc) => {
                        const updatedDoc = responseData.data.find((d: any) => d.id === doc.id);
                        return updatedDoc
                            ? {
                                ...doc,
    ...
    const canSignAll =
        (userRole === "Clerk" || userRole === "Judge") && hearing_status !== "completed";
                    ...
                    {localDocs.map((doc) => (
                        ...
                        >
                            <div className="flex items-center space-x-2">
                                ...
                                <span className="text-sm text-gray-600">({doc.document_status})</span>
                            </div>
                            ...
                            <div className="flex items-center space-x-4">
                                ...
                                {doc.verified_at ? (
                                    ...
                                    (userRole === "Registrar" && doc.document_status !== "verified") ||
                                        (userRole === "Judge" && !doc.verified_by_judge) ||
                                        (["Plaintiff", "Defendant", "Lawyer", "Prosecutor"].includes(userRole)) ? (
                                        <button
                                        ...
                                        <span className="text-sm text-gray-500">{doc.document_status}</span>
                                    )
                                )}
                            </div>
                        </div>
