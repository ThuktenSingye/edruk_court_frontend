...
interface RebutalTwoProps {
    caseId: string;
    onProceed?: () => void;
    title?: string;
}

export default function RebutalTwo({ caseId, onProceed, title = "REBUTTAL TWO" }: RebutalTwoProps) {
    ...
    useEffect(() => {
        const fetchRebuttalData = async () => {
            try {
                const response = await fetch(`http://nganglam.lvh.me:3001/api/v1/cases/${caseId}/rebuttal/2`, {
                    headers: {
