...
interface RebutalOneProps {
    caseId: string;
    onProceed?: () => void;
    title?: string;
}

export default function RebutalOne({ caseId, onProceed, title = "REBUTTAL ONE" }: RebutalOneProps) {
    ...
    useEffect(() => {
        const fetchRebuttalData = async () => {
            try {
                const response = await fetch(`http://nganglam.lvh.me:3001/api/v1/cases/${caseId}/rebuttal/1`, {
                    headers: {
