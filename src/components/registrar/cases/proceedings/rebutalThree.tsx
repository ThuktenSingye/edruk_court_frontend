...
interface RebutalThreeProps {
    caseId: string;
    onProceed?: () => void;
    title?: string;
}

export default function RebutalThree({ caseId, onProceed, title = "REBUTTAL THREE" }: RebutalThreeProps) {
    ...
    useEffect(() => {
        const fetchRebuttalData = async () => {
            try {
                const response = await fetch(`http://nganglam.lvh.me:3001/api/v1/cases/${caseId}/rebuttal/3`, {
                    headers: {
