...
export const CourtOrder = () => {
    ...
    const fetchSentCourtOrders = async () => {
      try {
        const response = await axios.get(
          `http://${host}:3001/api/v1/court_orders/sent`,
          {
            headers: {
  ...
  const handleSendToCourt = async (data: {
    ...
    if (data.court) {
      formData.append(
        "court_order[order_recipient_courts_attributes][0][court_id]",
        data.court
      );
    }
    ...
    if (data.user) {
      formData.append(
        "court_order[order_recipient_users_attributes][0][user_id]",
        data.user
      );
    }
            ...
            <TableBody>
              {paginatedOrders.map((order, index) => (
                <TableRow
                  ...
                  <TableCell>{startIndex + index + 1}</TableCell>
                  <TableCell>
                    {order.documents?.[0]?.filename || "N/A"}
                  </TableCell>
                      ...
                      <DialogContent className="max-w-4xl max-h-[90vh] overflow-auto space-y-6">
                        <div className="text-sm space-y-1">
                          ...
                          <p>
                            ...
                            {order.user_sender?.first_name}{" "}
                            {order.user_sender?.last_name}
                          </p>
                        </div>
