...
const CalendarEventSidebar: React.FC = () => {
    ...
    return (
        <div className="h-full max-h-[600px] overflow-y-auto border rounded-xl p-4 shadow-md bg-white flex flex-col">
            <h2 className="text-lg font-semibold mb-3 text-green-900">Hearing Schedule</h2>
                    ...
                    {events.map((event, index) => (
                        ...
                        >
                            <span className="flex w-full justify-between">
                                <p className="  text-md font-medium text-green-800 ">{event.hearing_type_name} hearing</p>
                            ...
                            <span>
                                ...
                                <p
                                    className={`text-sm pt-1 font-medium ${event.schedule_status === "pending" || event.schedule_status === "Cancelled"
                                            ? "text-[#AC3030]" // red
                                            : event.schedule_status === "Approved"
                                                ? "text-[#046200]" // green
                                                : event.schedule_status === "Changes_requested" || event.schedule_status === "Rescheduled"
                                                    ? "text-blue-600" // blue
                                                    : "text-gray-600"
                                        }`}
                                >
