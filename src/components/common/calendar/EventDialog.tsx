...
export default function EventDialog({
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange} modal={false}> {/* ✅ Important */}
            <DialogContent className="z-[9998]"> {/* ✅ Make sure it's not blocking picker */}
                <DialogHeader>
                    <DialogTitle className="text-[#046200]">{isRescheduling ? "Reschedule Event" : "Add Event"}</DialogTitle>
                </DialogHeader>
                <form onSubmit={onSubmit} className="space-y-4">
                    <Input placeholder="Case ID" value={caseId} onChange={(e) => setCaseId(e.target.value)} required />
                    <select className="w-full border p-2 rounded" value={hearingType} onChange={(e) => setHearingType(e.target.value)}>
