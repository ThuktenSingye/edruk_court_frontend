...
export default function NotificationPage() {
    ...
    const fetchNotifications = async () => {
      try {
        const response = await fetch(
          `http://${host}:3001/api/v1/notifications`,
          {
            headers: {
        ...
        console.log("notification date", jsonData);
        const formatted: Notification[] = Array.isArray(jsonData?.data)
          ? jsonData.data.map((item: any) => ({
              ...
              date: item.params?.hearing_schedule?.scheduled_date
                ? new Date(
                    item.params.hearing_schedule.scheduled_date
                  ).toLocaleString()
                : "No date",
              court_id:
                item.params?.case?.court_id !== undefined
  ...
  const markAsRead = async (notificationId: number, isWs: boolean = false) => {
    ...
    try {
      ...
      console.log("Mark as Read", response.status);
        ...
        {allNotifications.length > 0 ? (
          allNotifications.map((notification) => (
            <NotificationItem
              ...
              onMarkAsRead={() =>
                markAsRead(notification.id, notification.is_ws)
              }
              openPDF={openPDF}
            />
          ))
